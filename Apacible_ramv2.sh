...
# ============================================================
# Apacible Editor v2 — RAM Architecture
# A fast, in-memory terminal text editor in pure Bash.
#
...
ROWS=$(tput lines)
COLS=$(tput cols)
EDIT_ROWS=$(( ROWS - 2 ))  # Reserve bottom 2 rows: status + shortcuts
...
# Redraw all visible lines (flicker-free: overwrites in place, no full clear)
redraw_viewport() {
    local start_idx=$(( TOP_ROW - 1 ))
    local idx row visible

    for (( i=0; i<EDIT_ROWS; i++ )); do
        idx=$(( start_idx + i ))
        row=$(( i + 1 ))
        # Position at row start and clear the line
        echo -en "\e[${row};1H\e[2K"
        if (( idx < ${#FILE_LINES[@]} )); then
            # Print only the visible slice: COLS chars starting at LEFT_OFFSET
            visible="${FILE_LINES[$idx]:$LEFT_OFFSET:$COLS}"
            echo -en "${visible}"
        fi
    done
}

# Draw the status bar (inverse video) on the second-to-last row
draw_status_bar() {
    ...
    echo -en "\e[$((ROWS - 1));1H\e[7m${info:0:$COLS}\e[0m"
    ...
    echo -en "\e[${ROWS};1H\e[7m${keys:0:$COLS}\e[0m"
}
...
show_message() {
    ...
    echo -en "\e[$((ROWS - 1));1H\e[30;43m${msg:0:$COLS}\e[0m"
}
...
place_cursor() {
    ...
    echo -en "\e[${cursor_y};${screen_col}H"
}

# Full refresh: redraw viewport + status bar + place cursor
full_refresh() {
    adjust_scroll
    redraw_viewport
...
inline_refresh() {
    ...
    echo -en "\e[${cursor_y};1H\e[2K${FILE_LINES[$idx]:$LEFT_OFFSET:$COLS}"
    draw_status_bar
    place_cursor
}
