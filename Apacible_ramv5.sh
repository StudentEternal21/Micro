...
draw_title_bar() {
    ...
    # Calculate maximum available space for the filename
    local max_fname_len=$(( COLS - ${#left} - ${#right} - 2 ))
    
    # Truncate the filename if it exceeds the available space
    if (( ${#fname} > max_fname_len && max_fname_len > 3 )); then
        fname="${fname:0:$((max_fname_len - 3))}..."
    elif (( max_fname_len <= 3 )); then
        # Hide filename entirely if the terminal is too narrow
        fname="" 
    fi
